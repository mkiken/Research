  ...
  var result = {
    ...
    parse: function(input, startRule) {
      ...
      function parse_LineTerminatorSequence() {
        var cacheKey = "LineTerminatorSequence@" + pos.offset;
        var cachedResult = cache[cacheKey];
        if (cachedResult) {
          pos = clone(cachedResult.nextPos);
          return cachedResult.result;
      ...
      function parse_NullLiteral() {
        ...
        if (result0 !== null) {
          result0 = (function(offset, line, column) { return { type: "NullLiteral" }; })(pos0.offset, pos0.line, pos0.column);
        }
        if (result0 === null) {
          pos = clone(pos0);
        }
      ...
      function parse_LineContinuation() {
        var cacheKey = "LineContinuation@" + pos.offset;
        var cachedResult = cache[cacheKey];
        if (cachedResult) {
          pos = clone(cachedResult.nextPos);
          return cachedResult.result;
      ...
      function parse_RegularExpressionLiteral() {
        var cacheKey = "RegularExpressionLiteral@" + pos.offset;
        var cachedResult = cache[cacheKey];
        if (cachedResult) {
          pos = clone(cachedResult.nextPos);
          return cachedResult.result;
      ...
      function parse_RegularExpressionBody() {
        var cacheKey = "RegularExpressionBody@" + pos.offset;
        var cachedResult = cache[cacheKey];
        if (cachedResult) {
          pos = clone(cachedResult.nextPos);
          return cachedResult.result;
      ...
      function parse_RegularExpressionChars() {
        var cacheKey = "RegularExpressionChars@" + pos.offset;
        var cachedResult = cache[cacheKey];
        if (cachedResult) {
          pos = clone(cachedResult.nextPos);
          return cachedResult.result;
      ...
      function parse_RegularExpressionFirstChar() {
        var cacheKey = "RegularExpressionFirstChar@" + pos.offset;
        var cachedResult = cache[cacheKey];
        if (cachedResult) {
          pos = clone(cachedResult.nextPos);
          return cachedResult.result;
      ...
      function parse_RegularExpressionChar() {
        var cacheKey = "RegularExpressionChar@" + pos.offset;
        var cachedResult = cache[cacheKey];
        if (cachedResult) {
          pos = clone(cachedResult.nextPos);
          return cachedResult.result;
      ...
      function parse_RegularExpressionBackslashSequence() {
        var cacheKey = "RegularExpressionBackslashSequence@" + pos.offset;
        var cachedResult = cache[cacheKey];
        if (cachedResult) {
          pos = clone(cachedResult.nextPos);
          return cachedResult.result;
      ...
      function parse_RegularExpressionNonTerminator() {
        var cacheKey = "RegularExpressionNonTerminator@" + pos.offset;
        var cachedResult = cache[cacheKey];
        if (cachedResult) {
          pos = clone(cachedResult.nextPos);
          return cachedResult.result;
      ...
      function parse_RegularExpressionClass() {
        var cacheKey = "RegularExpressionClass@" + pos.offset;
        var cachedResult = cache[cacheKey];
        if (cachedResult) {
          pos = clone(cachedResult.nextPos);
          return cachedResult.result;
      ...
      function parse_RegularExpressionClassChars() {
        var cacheKey = "RegularExpressionClassChars@" + pos.offset;
        var cachedResult = cache[cacheKey];
        if (cachedResult) {
          pos = clone(cachedResult.nextPos);
          return cachedResult.result;
      ...
      function parse_RegularExpressionClassChar() {
        var cacheKey = "RegularExpressionClassChar@" + pos.offset;
        var cachedResult = cache[cacheKey];
        if (cachedResult) {
          pos = clone(cachedResult.nextPos);
          return cachedResult.result;
      ...
      function parse_RegularExpressionFlags() {
        var cacheKey = "RegularExpressionFlags@" + pos.offset;
        var cachedResult = cache[cacheKey];
        if (cachedResult) {
          pos = clone(cachedResult.nextPos);
          return cachedResult.result;
      ...
      function parse_Lm() {
        ...
        if (/^[\u02B0\u02B1\u02B2\u02B3\u02B4\u02B5\u02B6\u02B7\u02B8\u02B9\u02BA\u02BB\u02BC\u02BD\u02BE\u02BF\u02C0\u02C1\u02C6\u02C7\u02C8\u02C9\u02CA\u02CB\u02CC\u02CD\u02CE\u02CF\u02D0\u02D1\u02E0\u02E1\u02E2\u02E3\u02E4\u02EC\u02EE\u0374\u037A\u0559\u0640\u06E5\u06E6\u07F4\u07F5\u07FA\u0971\u0E46\u0EC6\u10FC\u17D7\u1843\u1C78\u1C79\u1C7A\u1C7B\u1C7C\u1C7D\u1D2C\u1D2D\u1D2E\u1D2F\u1D30\u1D31\u1D32\u1D33\u1D34\u1D35\u1D36\u1D37\u1D38\u1D39\u1D3A\u1D3B\u1D3C\u1D3D\u1D3E\u1D3F\u1D40\u1D41\u1D42\u1D43\u1D44\u1D45\u1D46\u1D47\u1D48\u1D49\u1D4A\u1D4B\u1D4C\u1D4D\u1D4E\u1D4F\u1D50\u1D51\u1D52\u1D53\u1D54\u1D55\u1D56\u1D57\u1D58\u1D59\u1D5A\u1D5B\u1D5C\u1D5D\u1D5E\u1D5F\u1D60\u1D61\u1D78\u1D9B\u1D9C\u1D9D\u1D9E\u1D9F\u1DA0\u1DA1\u1DA2\u1DA3\u1DA4\u1DA5\u1DA6\u1DA7\u1DA8\u1DA9\u1DAA\u1DAB\u1DAC\u1DAD\u1DAE\u1DAF\u1DB0\u1DB1\u1DB2\u1DB3\u1DB4\u1DB5\u1DB6\u1DB7\u1DB8\u1DB9\u1DBA\u1DBB\u1DBC\u1DBD\u1DBE\u1DBF\u2090\u2091\u2092\u2093\u2094\u2C7D\u2D6F\u2E2F\u3005\u3031\u3032\u3033\u3034\u3035\u303B\u309D\u309E\u30FC\u30FD\u30FE\uA015\uA60C\uA67F\uA717\uA718\uA719\uA71A\uA71B\uA71C\uA71D\uA71E\uA71F\uA770\uA788\uFF70\uFF9E\uFF9F]/.test(input.charAt(pos.offset))) {
          result0 = input.charAt(pos.offset);
          advance(pos, 1);
        } else {
          result0 = null;
      ...
      function parse_Lt() {
        ...
        if (/^[\u01C5\u01C8\u01CB\u01F2\u1F88\u1F89\u1F8A\u1F8B\u1F8C\u1F8D\u1F8E\u1F8F\u1F98\u1F99\u1F9A\u1F9B\u1F9C\u1F9D\u1F9E\u1F9F\u1FA8\u1FA9\u1FAA\u1FAB\u1FAC\u1FAD\u1FAE\u1FAF\u1FBC\u1FCC\u1FFC]/.test(input.charAt(pos.offset))) {
          result0 = input.charAt(pos.offset);
          advance(pos, 1);
        } else {
          result0 = null;
      ...
      function parse_Mc() {
        ...
        if (/^[\u0903\u093E\u093F\u0940\u0949\u094A\u094B\u094C\u0982\u0983\u09BE\u09BF\u09C0\u09C7\u09C8\u09CB\u09CC\u09D7\u0A03\u0A3E\u0A3F\u0A40\u0A83\u0ABE\u0ABF\u0AC0\u0AC9\u0ACB\u0ACC\u0B02\u0B03\u0B3E\u0B40\u0B47\u0B48\u0B4B\u0B4C\u0B57\u0BBE\u0BBF\u0BC1\u0BC2\u0BC6\u0BC7\u0BC8\u0BCA\u0BCB\u0BCC\u0BD7\u0C01\u0C02\u0C03\u0C41\u0C42\u0C43\u0C44\u0C82\u0C83\u0CBE\u0CC0\u0CC1\u0CC2\u0CC3\u0CC4\u0CC7\u0CC8\u0CCA\u0CCB\u0CD5\u0CD6\u0D02\u0D03\u0D3E\u0D3F\u0D40\u0D46\u0D47\u0D48\u0D4A\u0D4B\u0D4C\u0D57\u0D82\u0D83\u0DCF\u0DD0\u0DD1\u0DD8\u0DD9\u0DDA\u0DDB\u0DDC\u0DDD\u0DDE\u0DDF\u0DF2\u0DF3\u0F3E\u0F3F\u0F7F\u102B\u102C\u1031\u1038\u103B\u103C\u1056\u1057\u1062\u1063\u1064\u1067\u1068\u1069\u106A\u106B\u106C\u106D\u1083\u1084\u1087\u1088\u1089\u108A\u108B\u108C\u108F\u17B6\u17BE\u17BF\u17C0\u17C1\u17C2\u17C3\u17C4\u17C5\u17C7\u17C8\u1923\u1924\u1925\u1926\u1929\u192A\u192B\u1930\u1931\u1933\u1934\u1935\u1936\u1937\u1938\u19B0\u19B1\u19B2\u19B3\u19B4\u19B5\u19B6\u19B7\u19B8\u19B9\u19BA\u19BB\u19BC\u19BD\u19BE\u19BF\u19C0\u19C8\u19C9\u1A19\u1A1A\u1A1B\u1B04\u1B35\u1B3B\u1B3D\u1B3E\u1B3F\u1B40\u1B41\u1B43\u1B44\u1B82\u1BA1\u1BA6\u1BA7\u1BAA\u1C24\u1C25\u1C26\u1C27\u1C28\u1C29\u1C2A\u1C2B\u1C34\u1C35\uA823\uA824\uA827\uA880\uA881\uA8B4\uA8B5\uA8B6\uA8B7\uA8B8\uA8B9\uA8BA\uA8BB\uA8BC\uA8BD\uA8BE\uA8BF\uA8C0\uA8C1\uA8C2\uA8C3\uA952\uA953\uAA2F\uAA30\uAA33\uAA34\uAA4D]/.test(input.charAt(pos.offset))) {
          result0 = input.charAt(pos.offset);
          advance(pos, 1);
        } else {
          result0 = null;
      ...
      function parse_Nl() {
        ...
        if (/^[\u16EE\u16EF\u16F0\u2160\u2161\u2162\u2163\u2164\u2165\u2166\u2167\u2168\u2169\u216A\u216B\u216C\u216D\u216E\u216F\u2170\u2171\u2172\u2173\u2174\u2175\u2176\u2177\u2178\u2179\u217A\u217B\u217C\u217D\u217E\u217F\u2180\u2181\u2182\u2185\u2186\u2187\u2188\u3007\u3021\u3022\u3023\u3024\u3025\u3026\u3027\u3028\u3029\u3038\u3039\u303A]/.test(input.charAt(pos.offset))) {
          result0 = input.charAt(pos.offset);
          advance(pos, 1);
        } else {
          result0 = null;
      ...
      function parse_EOSNoLineTerminator() {
        var cacheKey = "EOSNoLineTerminator@" + pos.offset;
        var cachedResult = cache[cacheKey];
        if (cachedResult) {
          pos = clone(cachedResult.nextPos);
          return cachedResult.result;
      ...
      function parse_PrimaryExpression() {
        ...
        if (result0 === null) {
          ...
          if (result0 !== null) {
            result0 = (function(offset, line, column, name) { return { type: "Variable", name: name }; })(pos0.offset, pos0.line, pos0.column, result0);
          }
          if (result0 === null) {
            pos = clone(pos0);
          }
      ...
      function parse_PropertyAssignment() {
        ...
        if (result0 === null) {
          ...
          if (result0 !== null) {
            result0 = (function(offset, line, column, name, body) {
                return {
                  type: "GetterDefinition",
                  name: name,
                  body: body
                };
              })(pos0.offset, pos0.line, pos0.column, result0[2], result0[10]);
          }
          if (result0 === null) {
            pos = clone(pos0);
          }
          if (result0 === null) {
            ...
            if (result0 !== null) {
              result0 = (function(offset, line, column, name, param, body) {
                  return {
      ...
      function parse_MemberExpression() {
        ...
        if (result0 === null) {
          ...
          if (result0 === null) {
            ...
            if (result0 !== null) {
              result0 = (function(offset, line, column, constructor, arguments) {
                      return {
                        type:        "NewOperator",
      ...
      function parse_NewExpression() {
        ...
        if (result0 === null) {
          ...
          if (result0 !== null) {
            result0 = (function(offset, line, column, constructor) {
                return {
                  type:        "NewOperator",
                  constructor: constructor,
                  arguments:   []
      ...
      function parse_StatementList() {
        var cacheKey = "StatementList@" + pos.offset;
        var cachedResult = cache[cacheKey];
        if (cachedResult) {
          pos = clone(cachedResult.nextPos);
          return cachedResult.result;
      ...
      function parse_VariableStatement() {
        var cacheKey = "VariableStatement@" + pos.offset;
        var cachedResult = cache[cacheKey];
        if (cachedResult) {
          pos = clone(cachedResult.nextPos);
          return cachedResult.result;
      ...
      function parse_VariableDeclarationList() {
        var cacheKey = "VariableDeclarationList@" + pos.offset;
        var cachedResult = cache[cacheKey];
        if (cachedResult) {
          pos = clone(cachedResult.nextPos);
          return cachedResult.result;
      ...
      function parse_VariableDeclarationListNoIn() {
        var cacheKey = "VariableDeclarationListNoIn@" + pos.offset;
        var cachedResult = cache[cacheKey];
        if (cachedResult) {
          pos = clone(cachedResult.nextPos);
          return cachedResult.result;
      ...
      function parse_VariableDeclaration() {
        var cacheKey = "VariableDeclaration@" + pos.offset;
        var cachedResult = cache[cacheKey];
        if (cachedResult) {
          pos = clone(cachedResult.nextPos);
          return cachedResult.result;
      ...
      function parse_VariableDeclarationNoIn() {
        var cacheKey = "VariableDeclarationNoIn@" + pos.offset;
        var cachedResult = cache[cacheKey];
        if (cachedResult) {
          pos = clone(cachedResult.nextPos);
          return cachedResult.result;
      ...
      function parse_Initialiser() {
        var cacheKey = "Initialiser@" + pos.offset;
        var cachedResult = cache[cacheKey];
        if (cachedResult) {
          pos = clone(cachedResult.nextPos);
          return cachedResult.result;
      ...
      function parse_InitialiserNoIn() {
        var cacheKey = "InitialiserNoIn@" + pos.offset;
        var cachedResult = cache[cacheKey];
        if (cachedResult) {
          pos = clone(cachedResult.nextPos);
          return cachedResult.result;
      ...
      function parse_EmptyStatement() {
        var cacheKey = "EmptyStatement@" + pos.offset;
        var cachedResult = cache[cacheKey];
        if (cachedResult) {
          pos = clone(cachedResult.nextPos);
          return cachedResult.result;
        ...
        if (result0 !== null) {
          result0 = (function(offset, line, column) { return { type: "EmptyStatement" }; })(pos0.offset, pos0.line, pos0.column);
        }
        if (result0 === null) {
          pos = clone(pos0);
        }
      ...
      function parse_ExpressionStatement() {
        var cacheKey = "ExpressionStatement@" + pos.offset;
        var cachedResult = cache[cacheKey];
        if (cachedResult) {
          pos = clone(cachedResult.nextPos);
          return cachedResult.result;
      ...
      function parse_IfStatement() {
        var cacheKey = "IfStatement@" + pos.offset;
        var cachedResult = cache[cacheKey];
        if (cachedResult) {
          pos = clone(cachedResult.nextPos);
          return cachedResult.result;
      ...
      function parse_IterationStatement() {
        var cacheKey = "IterationStatement@" + pos.offset;
        var cachedResult = cache[cacheKey];
        if (cachedResult) {
          pos = clone(cachedResult.nextPos);
          return cachedResult.result;
      ...
      function parse_DoWhileStatement() {
        var cacheKey = "DoWhileStatement@" + pos.offset;
        var cachedResult = cache[cacheKey];
        if (cachedResult) {
          pos = clone(cachedResult.nextPos);
          return cachedResult.result;
      ...
      function parse_WhileStatement() {
        var cacheKey = "WhileStatement@" + pos.offset;
        var cachedResult = cache[cacheKey];
        if (cachedResult) {
          pos = clone(cachedResult.nextPos);
          return cachedResult.result;
      ...
      function parse_ForStatement() {
        var cacheKey = "ForStatement@" + pos.offset;
        var cachedResult = cache[cacheKey];
        if (cachedResult) {
          pos = clone(cachedResult.nextPos);
          return cachedResult.result;
      ...
      function parse_ForInStatement() {
        var cacheKey = "ForInStatement@" + pos.offset;
        var cachedResult = cache[cacheKey];
        if (cachedResult) {
          pos = clone(cachedResult.nextPos);
          return cachedResult.result;
          ...
          if (result1 !== null) {
            ...
            if (result2 !== null) {
              ...
              if (result3 !== null) {
                ...
                if (result4 !== null) {
                  result4 = (function(offset, line, column, declaration) { return declaration; })(pos2.offset, pos2.line, pos2.column, result4[2]);
                }
                if (result4 === null) {
                  pos = clone(pos2);
                }
        ...
        if (result0 !== null) {
          result0 = (function(offset, line, column, iterator, collection, statement) {
              return {
                type:       "ForInStatement",
                iterator:   iterator,
      ...
      function parse_ContinueStatement() {
        var cacheKey = "ContinueStatement@" + pos.offset;
        var cachedResult = cache[cacheKey];
        if (cachedResult) {
          pos = clone(cachedResult.nextPos);
          return cachedResult.result;
      ...
      function parse_BreakStatement() {
        var cacheKey = "BreakStatement@" + pos.offset;
        var cachedResult = cache[cacheKey];
        if (cachedResult) {
          pos = clone(cachedResult.nextPos);
          return cachedResult.result;
      ...
      function parse_ReturnStatement() {
        var cacheKey = "ReturnStatement@" + pos.offset;
        var cachedResult = cache[cacheKey];
        if (cachedResult) {
          pos = clone(cachedResult.nextPos);
          return cachedResult.result;
      ...
      function parse_WithStatement() {
        var cacheKey = "WithStatement@" + pos.offset;
        var cachedResult = cache[cacheKey];
        if (cachedResult) {
          pos = clone(cachedResult.nextPos);
          return cachedResult.result;
        ...
        if (result0 !== null) {
          result0 = (function(offset, line, column, environment, statement) {
              return {
                type:        "WithStatement",
      ...
      function parse_SwitchStatement() {
        var cacheKey = "SwitchStatement@" + pos.offset;
        var cachedResult = cache[cacheKey];
        if (cachedResult) {
          pos = clone(cachedResult.nextPos);
          return cachedResult.result;
      ...
      function parse_CaseBlock() {
        var cacheKey = "CaseBlock@" + pos.offset;
        var cachedResult = cache[cacheKey];
        if (cachedResult) {
          pos = clone(cachedResult.nextPos);
          return cachedResult.result;
      ...
      function parse_DefaultClause() {
        var cacheKey = "DefaultClause@" + pos.offset;
        var cachedResult = cache[cacheKey];
        if (cachedResult) {
          pos = clone(cachedResult.nextPos);
          return cachedResult.result;
      ...
      function parse_LabelledStatement() {
        var cacheKey = "LabelledStatement@" + pos.offset;
        var cachedResult = cache[cacheKey];
        if (cachedResult) {
          pos = clone(cachedResult.nextPos);
          return cachedResult.result;
      ...
      function parse_ThrowStatement() {
        var cacheKey = "ThrowStatement@" + pos.offset;
        var cachedResult = cache[cacheKey];
        if (cachedResult) {
          pos = clone(cachedResult.nextPos);
          return cachedResult.result;
      ...
      function parse_TryStatement() {
        var cacheKey = "TryStatement@" + pos.offset;
        var cachedResult = cache[cacheKey];
        if (cachedResult) {
          pos = clone(cachedResult.nextPos);
          return cachedResult.result;
        ...
        if (result0 === null) {
          ...
          if (result0 !== null) {
            result0 = (function(offset, line, column, block, catch_) {
                return {
                  type:      "TryStatement",
                  block:     block,
                  "catch":   catch_,
          ...
          if (result0 === null) {
            ...
            if (result0 !== null) {
              result0 = (function(offset, line, column, block, finally_) {
                  return {
                    type:      "TryStatement",
                    block:     block,
                    "catch":   null,
      ...
      function parse_Catch() {
        ...
        if (result0 !== null) {
          result0 = (function(offset, line, column, identifier, block) {
              return {
                type:       "Catch",
      ...
      function parse_Finally() {
        ...
        if (result0 !== null) {
          result0 = (function(offset, line, column, block) {
              return {
                type:  "Finally",
                block: block
              };
      ...
      function parse_DebuggerStatement() {
        var cacheKey = "DebuggerStatement@" + pos.offset;
        var cachedResult = cache[cacheKey];
        if (cachedResult) {
          pos = clone(cachedResult.nextPos);
          return cachedResult.result;
        ...
        if (result0 !== null) {
          result0 = (function(offset, line, column) { return { type: "DebuggerStatement" }; })(pos0.offset, pos0.line, pos0.column);
        }
        if (result0 === null) {
          pos = clone(pos0);
        }
      ...
      function parse_FunctionDeclaration() {
        var cacheKey = "FunctionDeclaration@" + pos.offset;
        var cachedResult = cache[cacheKey];
        if (cachedResult) {
          pos = clone(cachedResult.nextPos);
          return cachedResult.result;
      ...
      function parse_FunctionExpression() {
        var cacheKey = "FunctionExpression@" + pos.offset;
        var cachedResult = cache[cacheKey];
        if (cachedResult) {
          pos = clone(cachedResult.nextPos);
          return cachedResult.result;
      ...
      function parse_FormalParameterList() {
        var cacheKey = "FormalParameterList@" + pos.offset;
        var cachedResult = cache[cacheKey];
        if (cachedResult) {
          pos = clone(cachedResult.nextPos);
          return cachedResult.result;
